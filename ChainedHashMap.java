        ...
        for (AbstractIterableMap<K, V> chain : chains) {
            if (chain != null) {
                for (Entry<K, V> map: chain) {
                    if (map != null) { // added this if statement, IDK if it's necessary
                        int newIndex = Math.abs(map.getKey().hashCode()) % newChains.length;
                        // switched from newCap to newChains.length, and added Math.abs
